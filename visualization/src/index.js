const sd_client = require("sd-client");
...
const sd = new sd_client.AWClient("sd-watcher-input", {
  testing: testing,
  baseURL: url.origin,
});
...
function load() {
  ...
  const bucketName = `sd-watcher-input_${hostname}`;
  ...
  sd.getBuckets()
    ...
    .then(() => {
      return sd.getEvents(bucketName, { start: start, end: end });
    })
    .then((events) => {
